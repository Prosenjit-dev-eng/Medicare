      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Specialist Doctors <span className="text-emerald-600 dark:text-emerald-400">Directory</span>
          </h1>
        ...
        <button
          ...
          className="self-start sm:self-auto px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-emerald-300 text-slate-700 dark:text-slate-200 text-xs font-bold shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer"
        >
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {doctors.length === 0 ? (
                ...
                doctors.map((doc) => (
                  <tr key={doc._id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-4 px-4 font-bold text-slate-900 dark:text-white">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-slate-800 border border-emerald-200 dark:border-slate-700 flex items-center justify-center text-emerald-700 dark:text-emerald-400 font-bold overflow-hidden shrink-0">
                          {doc.imageUrl ? (
                    ...
                    <td className="py-4 px-4 text-slate-600 dark:text-slate-300">
                      ...
                      <span className="text-[11px] text-slate-400 dark:text-slate-500">{doc.experience || "8+ Years"} Experience</span>
                    </td>
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          title="View Profile"
                          className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 text-slate-600 dark:text-slate-300 hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors cursor-pointer"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-emerald-100 dark:border-slate-800 overflow-hidden">
            ...
            <div className="p-6 space-y-4 text-xs text-slate-600 dark:text-slate-300">
              <div>
                <strong className="text-slate-900 dark:text-white block mb-1 text-sm">Qualifications</strong>
                <p>{selectedDoctor.qualifications || "MBBS, MD"}</p>
              </div>

              <div>
                <strong className="text-slate-900 dark:text-white block mb-1 text-sm">Biography & About</strong>
                <p className="leading-relaxed">{selectedDoctor.about || "No biography provided."}</p>
              </div>

              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                <div>
                  <span className="text-slate-400 dark:text-slate-500 font-semibold block">Consult Fee</span>
                  <span className="text-base font-extrabold text-slate-900 dark:text-white">₹{selectedDoctor.fee || 500}</span>
                </div>
                <div>
                  <span className="text-slate-400 dark:text-slate-500 font-semibold block">Experience</span>
                  <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{selectedDoctor.experience || "8+ Years"}</span>
                </div>
              </div>
            </div>
          </div>
