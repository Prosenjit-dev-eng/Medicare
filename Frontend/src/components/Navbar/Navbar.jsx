...
function Navbar() {
  ...
  const { isSignedIn, user, openLogin, signOut } = useAuth();
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            {!isSignedIn ? (
              <button
                onClick={() => openLogin()}
                className="inline-flex items-center gap-1.5 px-5 py-2 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-bold shadow-xs hover:shadow-md transition-all cursor-pointer"
              >
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2">
              ...
              {!isSignedIn ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold shadow-xs cursor-pointer"
                >
                ...
                <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-emerald-600 text-white flex items-center justify-center text-xs font-bold">
                      {user?.name ? user.name.charAt(0).toUpperCase() : "P"}
                    </div>
                    <span className="text-xs font-bold text-slate-800 dark:text-white">{user?.name || "Patient"}</span>
                  </div>
                  <button
                    onClick={() => {
                      setIsOpen(false);
                    ...
                    className="text-xs font-bold text-red-500 hover:underline inline-flex items-center gap-1 cursor-pointer"
                  >
                    <LogOut className="w-3.5 h-3.5" />
                    <span>Sign Out</span>
                  </button>
